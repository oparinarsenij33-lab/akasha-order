...
window.showLibraryDepartment = async function(depId) {
    ...
    if (booksInDep.length === 0) {
        ...
        booksInDep.forEach(book => {
            ...
            if (book.coverUrl) html += `<img src="${book.coverUrl}" style="width:60px; height:80px; object-fit:cover; border-radius:5px; border:1px solid var(--border-color);">`;
            ...
            if (book.fileUrl) {
                ...
                if (isLink) html += `<a href="${book.fileUrl}" target="_blank" style="color:#64ffda; text-decoration:underline; font-size:0.9em;">🔗 Открыть книгу</a>`;
...
window.startAddDepartment = function() {
    addMessage(`<p>📚 <strong>Создание нового Отдела Библиотеки</strong></p><p>Введите <strong>название отдела</strong> (например: "Учебные материалы", "Художественная литература") или <em>"отмена"</em>:</p>`);
...
window.startAddBook = function(depId) {
    ...
    addLessonState = { step: 'add_book_cover', departmentId: depId, departmentName: dep.name };
};
...
async function handleBookCreation(input) {
    ...
    if (addLessonState.step === 'add_book_cover') {
        ...
        addLessonState.coverUrl = q === 'нет' ? '' : input;
    ...
    if (addLessonState.step === 'add_book_title') {
        if (q === 'отмена') { addLessonState = null; return '<p> Добавление отменено.</p>'; }
        addLessonState.title = q === 'нет' ? 'Без названия' : input;
    ...
    if (addLessonState.step === 'add_book_author') {
        if (q === 'отмена') { addLessonState = null; return '<p>❌ Добавление отменено.</p>'; }
        addLessonState.author = q === 'неизвестно' ? 'Неизвестный автор' : input;
    ...
    if (addLessonState.step === 'add_book_desc') {
        if (q === 'отмена') { addLessonState = null; return '<p> Добавление отменено.</p>'; }
        addLessonState.description = q === 'нет' ? '' : input;
    ...
    if (addLessonState.step === 'add_book_file') {
        ...
        if (q === 'файл') {
            ...
            addMessage(`<p> <strong>Загрузка файла книги</strong></p><p>Нажмите кнопку ниже, чтобы выбрать файл (PDF, EPUB, TXT и др.)</p><button class="hw-btn" onclick="window.uploadBookFile('${addLessonState.departmentId}')">📤 Выбрать файл</button><p>После загрузки напишите <em>"готово"</em> или <em>"отмена"</em>.</p>`);
            return '';
        }
    ...
    if (addLessonState.step === 'add_book_file_upload') {
        if (q === 'отмена') { addLessonState = null; return '<p>❌ Добавление отменено.</p>'; }
        if (q === 'готово') {
            await finalizeBookCreation(addLessonState.fileUrl || '', 'file_uploaded');
            return '';
        }
...
async function finalizeBookCreation(fileUrl, uploadMethod) {
    const success = await addBookToFirebase(addLessonState.departmentId, addLessonState.title, addLessonState.author, addLessonState.description, fileUrl, addLessonState.coverUrl || '');
    if (success) { 
...
window.uploadBookFile = function(departmentId) {
    ...
    fileInput.onchange = async (e) => {
        ...
        if (!file) {
            addMessage('<p>❌ Файл не выбран!</p>');
            return;
        }
        
        try {
            if (!storageRef) {
                addMessage('<p>❌ Firebase Storage не инициализирован! Попробуйте обновить страницу.</p>');
                return;
            }
            
            const bookRef = storageRef.child(`library/${departmentId}/${Date.now()}_${file.name}`);
            const uploadTask = bookRef.put(file);
            
            addMessage(`<p>📥 Начинаю загрузку файла "${file.name}"...</p>`);
            ...
            uploadTask.on('state_changed',
                ...
                async () => {
                    try {
                        ...
                        addMessage(`<p>✅ Файл успешно загружен!</p><p>URL: <code style="word-break: break-all; display: block; margin: 10px 0; padding: 10px; background: rgba(0,0,0,0.3); border-radius: 5px;">${downloadURL}</code></p><p>Напишите <em>"готово"</em> для завершения добавления книги.</p>`);
                    } catch (err) {
                        addMessage(`<p>❌ Ошибка получения URL: ${err.message}</p>`);
                    }
                }
            );
        } catch (error) {
            addMessage(`<p>❌ Ошибка: ${error.message}</p>`);
...
async function findAnswer(question) {
    ...
    if (addLessonState && addLessonState.step === 'add_book_cover') return handleBookCreation(question);
    if (addLessonState && addLessonState.step === 'add_book_title') return handleBookCreation(question);
    if (addLessonState && addLessonState.step === 'add_book_author') return handleBookCreation(question);
    if (addLessonState && addLessonState.step === 'add_book_desc') return handleBookCreation(question);
    if (addLessonState && addLessonState.step === 'add_book_file') return handleBookCreation(question);
    if (addLessonState && addLessonState.step === 'add_book_file_upload') return handleBookCreation(question);
...
window.addUserStatus = async function(userKey) { 
    ...
    if (!select) { showAlert('Ошибка', 'Элемент выбора статуса не найден. Обновите страницу.'); return; }
    ...
    if (!newStatus || newStatus === '' || newStatus === '-- Выберите статус --') { showAlert('Ошибка', 'Выберите статус из списка!'); return; } 
...
window.addUserTitle = async function(userKey) { 
    ...
    if (!select) { showAlert('Ошибка', 'Элемент выбора звания не найден. Обновите страницу.'); return; }
    const clarificationInput = document.getElementById('title-clarification-input'); 
    let newTitle = select.value; 
    if (!newTitle || newTitle === '' || newTitle === '-- Выберите звание --') { showAlert('Ошибка', 'Выберите звание из списка!'); return; } 
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (typeof firebase !== 'undefined' && firebaseConfig) { try { if (!firebase.apps.length) firebase.initializeApp(firebaseConfig); windowDb = firebase.firestore(); initFirebaseStorage(); console.log('✅ Firebase инициализирован'); } catch (e) { console.error('Ошибка инициализации Firebase:', e); } }
    setTimeout(async () => {
        if (windowDb) {
            loadUserFromStorage();
