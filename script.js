...
// =========================================================
// 🧹 ЕДИНСТВЕННЫЙ ХОЗЯИН КНОПОК РЕДАКТОРА (обнуляет все 9 блоков)
// Ставит на редактор ВСЕ чужие флаги -> 9 блоков больше не лезут.
// Кнопки клонируются (снимаются ВСЕ старые обработчики разом).
// Выделение читается НАПРЯМУЮ в момент тапа (preventDefault на touchstart
//   не даёт ему слететь) — никакой протухшей переменной, никакого ed.focus().
// Ж/К = ТОЛЬКО выделение (нет выделения = ничего не делает, весь текст цел).
// H1/H2/Ц/стрелки/❝/¶ = ОДНА строка-блок под курсором (не весь редактор).
// =========================================================
(function () {
  function edEl() { return document.getElementById('ak-fmt-editor'); }
  function selInEd(ed) {
    try {
      ...
      var st = r.startContainer;
      if (ed === anc || ed.contains(anc) || ed === st || ed.contains(st)) return r;
    } catch (e) {}
    return null;
  }
  function blockUnder(ed, node) {
    var n = node; if (n && n.nodeType === 3) n = n.parentNode;
    while (n && n !== ed && n.parentNode !== ed) n = n.parentNode;
    return (n && n !== ed) ? n : null;
  }
  function curBlock(ed) { var r = selInEd(ed); if (!r) return null; return blockUnder(ed, r.startContainer); }
  function setCur(node) { try { var s = window.getSelection(); var rr = document.createRange(); rr.selectNodeContents(node); rr.collapse(false); s.removeAllRanges(); s.addRange(rr); } catch (e) {} }
  function QSTYLE() { return 'border-left:3px solid #64ffda;margin:14px 0;padding:8px 14px;color:#cfe8d4;font-style:italic;background:rgba(100,255,218,0.06);border-radius:0 8px 8px 0;'; }
  function doBold(ed) { var r = selInEd(ed); if (!r || r.collapsed) return; var el = document.createElement('strong'); try { r.surroundContents(el); } catch (e) { try { var f = r.extractContents(); el.appendChild(f); r.insertNode(el); } catch (e2) {} } }
  function doItalic(ed) { var r = selInEd(ed); if (!r || r.collapsed) return; var el = document.createElement('em'); try { r.surroundContents(el); } catch (e) { try { var f = r.extractContents(); el.appendChild(f); r.insertNode(el); } catch (e2) {} } }
  function doAlign(ed, a) { var b = curBlock(ed); if (b) b.style.textAlign = a; }
  function doHeading(ed, tag) { var b = curBlock(ed); if (!b) return; if ((b.nodeName || '').toLowerCase() === tag) { var p = document.createElement('div'); while (b.firstChild) p.appendChild(b.firstChild); b.parentNode.replaceChild(p, b); setCur(p); } else { var h = document.createElement(tag); while (b.firstChild) h.appendChild(b.firstChild); b.parentNode.replaceChild(h, b); setCur(h); } }
  function doQuote(ed) { var b = curBlock(ed); if (!b) return; if ((b.nodeName || '').toLowerCase() === 'blockquote') { var p = document.createElement('div'); while (b.firstChild) p.appendChild(b.firstChild); b.parentNode.replaceChild(p, b); setCur(p); } else { var q = document.createElement('blockquote'); q.style.cssText = QSTYLE(); while (b.firstChild) q.appendChild(b.firstChild); b.parentNode.replaceChild(q, b); setCur(q); } }
  function doIndent(ed) { var b = curBlock(ed); if (!b) return; var c = b.style.textIndent; b.style.textIndent = (c && c !== '0px' && c !== '0') ? '0' : '1.5em'; }
  var ACT = {
    'Ж': doBold, 'К': doItalic,
    'Ц': function (ed) { doAlign(ed, 'center'); }, '⬅': function (ed) { doAlign(ed, 'left'); }, '➡': function (ed) { doAlign(ed, 'right'); }, '↔': function (ed) { doAlign(ed, 'justify'); },
    'H1': function (ed) { doHeading(ed, 'h2'); }, 'H2': function (ed) { doHeading(ed, 'h3'); },
    '❝': doQuote, '¶': doIndent
  };
  function own(ed) {
    if (!ed || ed.getAttribute('data-ak-cleanfinal')) return;
    ed.setAttribute('data-ak-cleanfinal', '1');
    ed.setAttribute('data-ak-rewired', '1');
    ed.setAttribute('data-ak-rewired3', '1');
    ed.setAttribute('data-ak-rewired4', '1');
    ed.setAttribute('data-ak-rewired6', '1');
    var btns = ed.querySelectorAll('button');
    for (var i = 0; i < btns.length; i++) {
      var t = (btns[i].textContent || '').trim();
      var fn = ACT[t]; if (!fn) continue;
      var clone = btns[i].cloneNode(true);
      var fire = (function (f) { return function (ev) { if (ev) { ev.preventDefault(); ev.stopPropagation(); } try { f(ed); } catch (e) {} }; })(fn);
      clone.addEventListener('touchstart', fire, { passive: false });
