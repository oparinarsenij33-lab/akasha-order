...
const usersDatabase = {
    ...
    'далисса вестуро': { ранг: 'старший падаван', учитель: 'Аранэль Хальдарон', пароль: 'D5i10V3', fullName: 'Далисса Иденааль Вестуро', статусы: [], звания: [] },
...
function isArchivist() { return currentUser && hasStatus(currentUser, 'Архивариус'); }
...
async function addBookToFirebase(departmentId, title, author, description, content) {
    if (!windowDb) return false;
    try {
        ...
        const docRef = await windowDb.collection('library_books').add({
            departmentId: departmentId, title: title, author: author || 'Неизвестный автор',
            description: description || '', content: content || '', number: number,
            createdAt: firebase.firestore.Timestamp.fromDate(new Date()),
            addedBy: currentUser.name
        });
        const newBook = { id: docRef.id, departmentId: departmentId, title: title, author: author || 'Неизвестный автор', description: description || '', content: content || '', number: number };
...
window.showLibraryDepartment = async function(depId) {
    ...
    if (booksInDep.length === 0) {
        ...
        booksInDep.forEach(book => {
            html += `<div style="background:rgba(0,0,0,0.2); border-radius:8px; padding:12px; margin:8px 0; display:flex; justify-content:space-between; align-items:center; gap:10px;">`;
            ...
            if (book.description) html += `<div style="color:var(--text-secondary); font-size:0.85em; margin-top:3px;">${book.description.substring(0, 80)}${book.description.length > 80 ? '...' : ''}</div>`;
            html += `</div>`;
            if (isArchivist() || isAdmin()) {
                html += `<button onclick="window.deleteBook('${book.id}', '${book.title.replace(/'/g, "\\'")}')" style="background:rgba(255,80,80,0.2); color:#ff6b6b; padding:8px 12px; border-radius:6px; border:1px solid rgba(255,80,80,0.4); font-size:0.9em; min-width:40px;">️</button>`;
            }
            html += `</div>`;
        });
        html += `</div>`;
    ...
    if (isArchivist() || isAdmin()) {
        html += `<button class="hw-btn" onclick="window.startAddBook('${depId}')" style="width:100%; margin-top:20px; background:rgba(76,175,80,0.3); color:#4caf50; font-size:1.1em;">➕ Добавить книгу в этот отдел</button>`;
        html += `<button class="hw-btn" onclick="window.deleteDepartment('${depId}', '${dep.name.replace(/'/g, "\\'")}')" style="width:100%; margin-top:10px; background:rgba(255,80,80,0.2); color:#ff6b6b; font-size:1.1em;">️ Удалить весь отдел</button>`;
    }
...
window.showBookDetails = function(bookId) {
    ...
    if (!book) { addMessage('<p>❌ Книга не найдена!</p>'); return; }
    const dep = libraryDepartments.find(d => d.id === book.departmentId);
    let html = `<div style="background:rgba(13,31,15,0.5); border:1px solid var(--border-color); border-radius:15px; padding:25px; margin:15px 0;">`;
    html += `<h3 style="color:#64ffda; margin-bottom:10px; font-family:'Playfair Display',serif; text-align:center; font-size:1.6em;">📖 №${book.number}. ${book.title}</h3>`;
    html += `<p style="color:#8bc34a; text-align:center; font-size:1.1em; margin-bottom:15px;">✍️ ${book.author}</p>`;
    if (dep) html += `<p style="color:#6b5f4a; text-align:center; font-style:italic; margin-bottom:20px;">📚 Отдел: ${dep.name}</p>`;
    if (book.description) html += `<div style="background:rgba(0,0,0,0.2); border-radius:8px; padding:15px; margin-bottom:20px;"><p style="color:var(--text-color); line-height:1.6;"><strong>Описание:</strong> ${book.description}</p></div>`;
    if (book.content) {
        html += `<div style="background:rgba(0,0,0,0.3); border-radius:8px; padding:20px; margin-bottom:20px; max-height:400px; overflow-y:auto;">`;
        html += `<p style="color:var(--text-color); line-height:1.8; white-space:pre-wrap;">${book.content}</p>`;
        html += `</div>`;
    } else {
        html += `<p style="color:#6b5f4a; text-align:center; font-style:italic;">Содержимое книги отсутствует.</p>`;
    }
    html += `<button class="hw-btn" onclick="window.showLibraryDepartment('${book.departmentId}')" style="width:100%; margin-top:15px; padding:12px; font-size:1.1em;">🔙 Назад к отделу</button></div>`;
    addRawHTML(html);
};

window.startAddDepartment = function() {
    addMessage(`<p>📚 <strong>Создание нового Отдела Библиотеки</strong></p>
                <p>Введите <strong>название отдела</strong> (например: "Учебные материалы", "Художественная литература") или <em>"отмена"</em>:</p>`);
...
window.startAddBook = function(depId) {
    ...
    addMessage(`<p>📖 <strong>Добавление книги в "${dep.name}"</strong></p>
                <p>Введите <strong>название книги</strong> или <em>"отмена"</em>:</p>`);
    addLessonState = { step: 'add_book_title', departmentId: depId, departmentName: dep.name };
};
...
window.deleteDepartment = async function(depId, depName) {
    ...
    if (success) {
        showAlert('Успех', `Отдел "${depName}" удалён!`);
        window.showLibrary();
    } else {
        showAlert('Ошибка', 'Не удалось удалить отдел.');
    }
};
...
async function findAnswer(question) {
    ...
    if (addLessonState && addLessonState.step === 'add_department_desc') {
        ...
        if (success) {
            addMessage(`<p>✅ Отдел "${addLessonState.name}" создан!</p>`);
            await loadLibraryFromFirebase();
            window.showLibrary();
    ...
    if (addLessonState && addLessonState.step === 'add_book_title') {
        if (q === 'отмена') { addLessonState = null; return '<p>❌ Добавление отменено.</p>'; }
        addLessonState = { step: 'add_book_author', title: question };
        return '<p>Введите <strong>автора книги</strong> (или <em>"неизвестно"</em>, <em>"отмена"</em>):</p>';
    }
    if (addLessonState && addLessonState.step === 'add_book_author') {
        if (q === 'отмена') { addLessonState = null; return '<p>❌ Добавление отменено.</p>'; }
        const author = q === 'неизвестно' ? 'Неизвестный автор' : question;
        addLessonState = { step: 'add_book_desc', author: author };
        return '<p>Введите <strong>краткое описание книги</strong> (или <em>"нет"</em>, <em>"отмена"</em>):</p>';
    }
    if (addLessonState && addLessonState.step === 'add_book_desc') {
        if (q === 'отмена') { addLessonState = null; return '<p>❌ Добавление отменено.</p>'; }
        const desc = q === 'нет' ? '' : question;
        addLessonState = { step: 'add_book_content', description: desc };
        return '<p>Введите <strong>содержимое книги</strong> (полный текст) или <em>"нет"</em> / <em>"отмена"</em>:</p>';
    }
    if (addLessonState && addLessonState.step === 'add_book_content') {
        if (q === 'отмена') { addLessonState = null; return '<p> Добавление отменено.</p>'; }
        const content = q === 'нет' ? '' : question;
        const success = await addBookToFirebase(addLessonState.departmentId, addLessonState.title, addLessonState.author, addLessonState.description, content);
        if (success) {
            addMessage(`<p>✅ Книга "${addLessonState.title}" добавлена!</p>`);
            await loadLibraryFromFirebase();
            window.showLibraryDepartment(addLessonState.departmentId);
        } else addMessage('<p>❌ Ошибка добавления книги.</p>');
        addLessonState = null;
        return '';
    }
...
function toggleKeyboardVisibility() { const keyboard = document.getElementById('custom-keyboard'); const inputWrapper = document.getElementById('main-input-wrapper'); if (keyboard.style.display === 'none') { keyboard.style.display = 'flex'; inputWrapper.classList.remove('keyboard-hidden'); localStorage.setItem('akasha-keyboard-visible', 'true'); } else { keyboard.style.display = 'none'; inputWrapper.classList.add('keyboard-hidden'); localStorage.setItem('akasha-keyboard-visible', 'false'); } }
...
window.changeUserRank = async function(userKey) { const select = document.getElementById('rank-select'); const newRank = select.value; let teacherInput = ''; if (newRank === 'падаван' || newRank === 'старший падаван') { const teacherField = document.getElementById('teacher-input-field'); if (teacherField) { teacherInput = teacherField.value.trim(); if (!teacherInput) { showAlert('Ошибка', 'Для ранга Падаван или Старший Падаван необходимо указать Учителя!'); return; } } } try { const userRef = windowDb.collection('users').doc(userKey); const userDoc = await userRef.get(); if (userDoc.exists) { const updates = { rank: newRank }; if (teacherInput) updates.teacher = teacherInput.toLowerCase() === 'нет' || teacherInput.toLowerCase() === 'отсутствует' ? 'отсутствует' : teacherInput; await userRef.update(updates); } else { const user = usersDatabase[userKey]; await userRef.set({ fullName: user.fullName, rank: newRank, teacher: teacherInput || (user.учитель || 'отсутствует'), password: user.пароль || '', specialTitle: user.specialTitle || '', description: user.description || '', статусы: user.статусы || [], звания: user.звания || [], createdAt: firebase.firestore.Timestamp.fromDate(new Date()), createdBy: currentUser.name }); } usersDatabase[userKey].ранг = newRank; if (teacherInput) usersDatabase[userKey].учитель = teacherInput.toLowerCase() === 'нет' || teacherInput.toLowerCase() === 'отсутствует' ? 'отсутствует' : teacherInput; showAlert('Успех', `Ранг пользователя изменён на "${newRank}"!`); window.manageUserRanks(userKey); } catch (error) { showAlert('Ошибка', `Не удалось изменить ранг: ${error.message}`); } };
window.addUserStatus = async function(userKey) { const select = document.getElementById('status-select'); const councilInput = document.getElementById('council-name-input'); const customInput = document.getElementById('custom-status-input'); let newStatus = select.value; if (!newStatus) { showAlert('Ошибка', 'Выберите статус из списка!'); return; } if (newStatus === 'Член Совета') { const councilName = councilInput.value.trim(); if (!councilName) { showAlert('Ошибка', 'Введите название Совета!'); return; } newStatus = `Член Совета (${councilName})`; } else if (newStatus === 'Другие') { const customStatus = customInput.value.trim(); if (!customStatus) { showAlert('Ошибка', 'Введите свой статус!'); return; } newStatus = customStatus; } try { const userRef = windowDb.collection('users').doc(userKey); const userDoc = await userRef.get(); const currentStatuses = userDoc.data().статусы || []; await userRef.update({ статусы: [...currentStatuses, newStatus] }); if (!usersDatabase[userKey].статусы) usersDatabase[userKey].статусы = []; usersDatabase[userKey].статусы.push(newStatus); showAlert('Успех', `Статус "${newStatus}" добавлен!`); window.manageUserRanks(userKey); } catch (error) { showAlert('Ошибка', `Не удалось добавить статус: ${error.message}`); } };
window.removeUserStatus = async function(userKey, index) { try { const userRef = windowDb.collection('users').doc(userKey); const userDoc = await userRef.get(); const currentStatuses = userDoc.data().статусы || []; const newStatuses = currentStatuses.filter((_, i) => i !== index); await userRef.update({ статусы: newStatuses }); usersDatabase[userKey].статусы = newStatuses; showAlert('Успех', 'Статус удалён!'); window.manageUserRanks(userKey); } catch (error) { showAlert('Ошибка', `Не удалось удалить статус: ${error.message}`); } };
window.addUserTitle = async function(userKey) { const select = document.getElementById('title-select'); const clarificationInput = document.getElementById('title-clarification-input'); let newTitle = select.value; if (!newTitle) { showAlert('Ошибка', 'Выберите звание из списка!'); return; } const needsClarification = ['Рыцарь', 'Мастер', 'Предвестник', 'Вестник', 'Лорд', 'Леди']; let уточнение = ''; if (needsClarification.includes(newTitle)) { уточнение = clarificationInput.value.trim(); if (!уточнение) { showAlert('Ошибка', 'Введите уточнение звания!'); return; } } try { const userRef = windowDb.collection('users').doc(userKey); const userDoc = await userRef.get(); const currentTitles = userDoc.data().звания || []; await userRef.update({ звания: [...currentTitles, {звание: newTitle, уточнение: уточнение}] }); if (!usersDatabase[userKey].звания) usersDatabase[userKey].звания = []; usersDatabase[userKey].звания.push({звание: newTitle, уточнение: уточнение}); const titleDisplay = уточнение ? `${newTitle} (${уточнение})` : newTitle; showAlert('Успех', `Звание "${titleDisplay}" добавлено!`); window.manageUserRanks(userKey); } catch (error) { showAlert('Ошибка', `Не удалось добавить звание: ${error.message}`); } };
window.removeUserTitle = async function(userKey, index) { try { const userRef = windowDb.collection('users').doc(userKey); const userDoc = await userRef.get(); const currentTitles = userDoc.data().звания || []; const newTitles = currentTitles.filter((_, i) => i !== index); await userRef.update({ звания: newTitles }); usersDatabase[userKey].звания = newTitles; showAlert('Успех', 'Звание удалено!'); window.manageUserRanks(userKey); } catch (error) { showAlert('Ошибка', `Не удалось удалить звание: ${error.message}`); } };

document.addEventListener('DOMContentLoaded', async () => {
    if (isInitialized) return; isInitialized = true;
    applySeasonTheme(); renderKeyboard();
    if (typeof firebase !== 'undefined' && firebaseConfig) { try { if (!firebase.apps.length) firebase.initializeApp(firebaseConfig); windowDb = firebase.firestore(); console.log('✅ Firebase инициализирован'); } catch (e) { console.error('Ошибка инициализации Firebase:', e); } }
    setTimeout(async () => {
        if (windowDb) {
            loadUserFromStorage();
