...
bubble.innerHTML = `<div class="bubble-text"><img src="${msg.mediaUrl}" style="max-width:100%; border-radius:8px; cursor:pointer;" onclick="window.openImage('${msg.mediaUrl}')"><div style="margin-top:5px; font-size:0.85em; color:#a89b7e;">${msg.fileName || ''}</div></div><div class="bubble-time">${time}</div>`;
} else if (msg.mediaType === 'video') {
bubble.innerHTML = `<div class="bubble-text"><video src="${msg.mediaUrl}" controls style="max-width:100%; border-radius:8px;"></video><div style="margin-top:5px; font-size:0.85em; color:#a89b7e;">${msg.fileName || ''}</div></div><div class="bubble-time">${time}</div>`;
} else if (msg.mediaType === 'voice') {
bubble.innerHTML = `<div class="bubble-text"><audio src="${msg.mediaUrl}" controls style="max-width:100%;"></audio></div><div class="bubble-time">${time}</div>`;
} else {
bubble.innerHTML = `<div class="bubble-text">${msg.text}</div><div class="bubble-time">${time}</div>`;
}
} else {
...
// 🔥 ФУНКЦИИ ДЛЯ ОТПРАВКИ МЕДИА (ФОТО/ВИДЕО/ГОЛОС)
...
<button onclick="window.confirmSendMedia('${url}', '${type}')" style="flex:1; background:rgba(76,175,80,0.3); color:#4caf50; padding:10px; border-radius:6px; border:none; cursor:pointer;">✅ Отправить</button>
...
window.confirmSendMedia = async function(url, type) {
const preview = document.getElementById('media-preview');
if (preview) preview.innerHTML = '<p style="color:#64ffda; text-align:center;">⏳ Загрузка...</p>';
try {
const fileInput = document.querySelector('#media-preview input[type="file"]') || document.querySelector('input[type="file"]');
const file = fileInput?.files[0];
if (!file) {
const response = await fetch(url);
const blob = await response.blob();
const fileName = `chat_${Date.now()}.${type === 'photo' ? 'jpg' : 'mp4'}`;
const ref = storageRef.child(`chat_media/${fileName}`);
await ref.put(blob);
const downloadURL = await ref.getDownloadURL();
await sendMediaToChat(downloadURL, type, fileName);
} else {
...
const preview = document.getElementById('media-preview');
if (preview) preview.innerHTML = `<p style="color:#ff6b6b;"> Ошибка: ${error.message}</p>`;
}
};
...
to: window.currentChatPartner || currentUser.учитель,
text: '',
mediaUrl: mediaUrl,
mediaType: type,
fileName: fileName,
...
await windowDb.collection('messages').add(messageData);
if (window.currentChatPartner) {
await window.openChatWithStudent(window.currentChatPartner);
} else {
const masterName = currentUser.учитель;
...
indicator.innerHTML = `
<div style="background:rgba(255,107,107,0.2); border:1px solid rgba(255,107,107,0.5); border-radius:10px; padding:15px; margin:10px 0; text-align:center;">
<p style="color:#ff6b6b; margin:0; font-size:1.1em;">🔴 Запись...</p>
<p style="color:#a89b7e; margin:5px 0 0 0; font-size:0.9em;">Нажми 🛑 чтобы остановить</p>
</div>
`;
const container = document.getElementById('chat-container');
...
const preview = document.getElementById('media-preview');
if (preview) preview.innerHTML = '<p style="color:#64ffda; text-align:center;">⏳ Загрузка голосового сообщения...</p>';
try {
...
if (chatWrapper && !document.getElementById('media-chat-buttons')) {
...
// 🔥 ЧЛЕНЫ ОРДЕНА С ПУБЛИЧНЫМИ СТАТУСАМИ И ЗВАНИЯМИ
