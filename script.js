    ...
    if (assignmentsList.length === 0) {
        ...
        assignmentsList.forEach((hw) => {
            ...
            if (myLastSubmission) {
                ...
                // Кнопка удаления ответа для ученика
            ...
            // Блок для Мастеров — проверка ответов + удаление задания
            if (isMaster()) {
                ...
                html += `<button class="hw-btn" onclick="window.deleteAssignment('${hwId}', '${hwTitle.replace(/'/g, "\\'")}')" 
                         style="flex:1; min-width:150px; background:rgba(255,80,80,0.2); color:#ff6b6b; border:1px solid rgba(255,80,80,0.4);">
                         🗑️ Удалить задание</button>`;
                html += `</div></div>`;
            }
            // Кнопка отправки ответа (для всех, кроме Мастеров, если задание не их)
...
// ===== НОВАЯ ФУНКЦИЯ: УДАЛЕНИЕ ЗАДАНИЯ ДЛЯ МАСТЕРОВ =====
window.deleteAssignment = async function(assignmentId, assignmentTitle) {
    ...
    try {
        ...
        // Удаляем все ответы на это задание
...
function findAnswer(question) {
    ...
    if (!currentUser) {
        // Проверяем наличие запятых или ключевых слов для входа
        if (question.includes(',') || q.includes('имя') || q.includes('зовут') || q.includes('ранг') || q.includes('пароль') || q.includes('учитель')) {
