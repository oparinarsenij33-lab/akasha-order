...
const coverHtml = book.coverUrl && book.coverUrl.trim() !== '' ? `<img src="${book.coverUrl}" onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex';" style="width:60px;height:80px;object-fit:cover;border-radius:5px;border:1px solid var(--border-color);flex-shrink:0;background:#1a3a1a;">` + `<div style="display:none;width:60px;height:80px;border-radius:5px;border:1px solid var(--border-color);flex-shrink:0;">${bookIcon}</div>` : `<div style="width:60px;height:80px;border-radius:5px;border:1px solid var(--border-color);flex-shrink:0;display:flex;align-items:center;justify-content:center;background:#1a3a1a;">${bookIcon}</div>`;
html += coverHtml;
...
html += `<hr style="border-color:var(--border-color);margin:20px 0;"><p style="color:var(--text-secondary);text-align:center;margin-bottom:20px;font-style:italic;">Выбери раздел или задай вопрос Акаше</p></div>`;
...
if (isMaster()) html += `<button class="hw-btn" onclick="window.startAddLessonToSection('${section.id}')" style="width:100%;margin-top:20px;background:rgba(76,175,80,0.3);color:#4caf50;font-size:1.1em;">➕ Добавить урок в этот раздел</button>`;
...
else if (lesson.mediaUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i)) html += `<img src="${lesson.mediaUrl}" style="max-width:100%;max-height:300px;border-radius:10px;margin-top:10px;">`;
...
row.forEach(key => {
const keyDiv = document.createElement('div'); keyDiv.className = '
