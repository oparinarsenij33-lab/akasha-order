...
const usersDatabase = {
    ...
    'нарнэлион эдрад': { ранг: 'мастер', учитель: 'отсутствует', пароль: 'N1E1', fullName: 'Нарнэлион Эдрад', specialTitle: 'Мастер Артефактов и Целительства', description: 'Мастер Артефактов', статусы: ['Целитель', 'Исследователь'], звания: [{звание: 'Мастер', уточнение: 'Артефактов'}] },
...
function getStrangerGreeting() {
    return `<div style="background:rgba(13,31,15,0.5); border:1px solid var(--border-color); border-radius:15px; padding:25px; margin:15px 0;">
        ...
        <p style="color:var(--text-color); line-height:1.8; margin-bottom:15px;">Орден Вольных Джедаев — это братство тех, кто посвятил себя изучению высших искусств.</p>
        <h4 style="color:#8bc34a; margin:20px 0 10px 0; font-family:'Playfair Display',serif;">📋 Как получить доступ:</h4>
        <p style="color:var(--text-color); line-height:1.8; margin-bottom:15px;">Назови мне своё <strong>Имя</strong>, <strong>Ранг</strong>, имя своего <strong>Учителя</strong> и <strong>Пароль</strong>.<br><br><em>Пример:</em> "Меня зовут Оби-Ван Кеноби, я Магистр, мой Учитель — Квай-Гон Джинн, пароль O2V7K9"</p>
...
// ===== ЗАГРУЗКА РАЗДЕЛОВ (с localStorage!) =====
...
// ===== ДОБАВЛЕНИЕ РАЗДЕЛА (с localStorage!) =====
...
async function updateOnlineStatus() {
    ...
    try {
        ...
        await windowDb.collection('online_status').doc(currentUser.name).set({
            lastSeen: now, online: true, updatedAt: now
        }, { merge: true });
...
window.startAddSchedule = function() {
    addMessage(`<p>📅 <strong>Добавление занятия в расписание</strong></p><p>Введите <strong>дату и время</strong> в формате: <em>ГГГГ-ММ-ДД ЧЧ:ММ</em><br>Например: <em>2026-07-20 18:00</em> (это будет 20 июля 2026, 18:00 МСК)<br>(или <em>"отмена"</em>)</p>`);
...
//  ОГЛАВЛЕНИЕ — ТРЕХУРОВНЕВОЕ (Год → Раздел → Уроки)
window.showTOC = async function() {
    ...
    if (years.length === 0) {
        ...
        if (isAdmin()) html += `<button class="hw-btn" onclick="window.startAddYear()" style="width:100%; margin-top:20px; background:rgba(76,175,80,0.3); color:#4caf50;"> Добавить новый год</button>`;
    } else {
        html += `<div style="margin-bottom:20px;">`;
        years.forEach(year => { html += `<button class="hw-btn" onclick="window.showYearSections(${year})" style="width:100%; margin-bottom:10px; background:rgba(100,255,218,0.2); color:#64ffda; font-size:1.2em;"> ${year} год</button>`; });
        html += `</div>`;
        if (isAdmin()) html += `<button class="hw-btn" onclick="window.startAddYear()" style="width:100%; margin-top:20px; background:rgba(76,175,80,0.3); color:#4caf50;"> Добавить новый год</button>`;
    }
    html += `<button class="hw-btn" onclick="showMainMenu()" style="width:100%; margin-top:15px; padding:12px;">🔙 Вернуться в меню</button></div>`;
...
window.showYearSections = async function(year) {
    ...
    if (yearSections.length === 0) html += `<p style="color:#6b5f4a; text-align:center; font-style:italic;">В этом году разделов пока нет.</p>`;
    else {
        html += `<div style="margin-bottom:20px;">`;
        yearSections.forEach(section => {
            if (availableRanks.includes(section.rank)) html += `<button class="hw-btn" onclick="window.showSectionLessons('${section.id}')" style="width:100%; margin-bottom:10px; background:rgba(139,195,74,0.2); color:var(--accent-color); font-size:1.1em;">📖 ${section.name}</button>`;
        });
        html += `</div>`;
    }
    if (isAdmin()) html += `<button class="hw-btn" onclick="window.startAddSection(${year})" style="width:100%; margin-top:20px; background:rgba(76,175,80,0.3); color:#4caf50;">➕ Добавить раздел в ${year} год</button>`;
...
window.showSectionLessons = async function(sectionId) {
    ...
    if (sectionLessons.length === 0) html += `<p style="color:#6b5f4a; text-align:center; font-style:italic;">Уроков в этом разделе пока нет.</p>`;
    else {
        html += `<div style="margin-bottom:20px;">`;
        sectionLessons.forEach(lesson => { html += `<div class="toc-lesson-link" onclick="window.showLessonContent('${lesson.id}')" style="padding:10px; margin:5px 0; background:rgba(0,0,0,0.2); border-radius:8px;">📚 ${lesson.title}</div>`; });
        html += `</div>`;
    }
    if (isMaster()) html += `<button class="hw-btn" onclick="window.startAddLessonToSection('${sectionId}')" style="width:100%; margin-top:20px; background:rgba(76,175,80,0.3); color:#4caf50;">➕ Добавить урок в этот раздел</button>`;
        ...
        if (question.includes(',') || q.includes('имя') || q.includes('ранг') || q.includes('пароль') || q.includes('учитель')) {
            ...
            if (userData.name && userData.ранг && userData.пароль) {
                ...
                if (foundUser && foundUser.ранг === userData.ранг && foundUser.пароль === userData.пароль) {
                    ...
                    loadLessonsFromFirebase(); loadAssignments(); loadSubmissions(); loadScheduleFromFirebase(); loadSectionsFromFirebase(); loadOnlineStatuses(); registerUserIfNeeded();
